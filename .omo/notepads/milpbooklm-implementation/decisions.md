...
## D18 (2026-09-22, task 28) — RSR-01c user-space browser worker form
- The plan's rootless-container worker (Playwright 1.63-pinned Chromium image) is T8-class on this host (no container runtime as this user); the delivered form is a Node subprocess (existing product-E2E Playwright install, 1.58.2/Chromium 145) driven by the Python adapter over line-JSON stdio. Container-form pin reconciliation (1.63/Chromium 153) is recorded for when T8 unblocks.
- T22 addressing tables are single-sourced: `browser_worker.py` exports `_IPV4_BLOCKED`/`_IPV6_BLOCKED` as integer ranges in the worker init handshake; the JS side does pure range matching (v4 int math, v6 BigInt, v4-mapped unwrap). No forked CIDR table in JS.
- Loopback tension resolved explicitly: `allowed_origins` is an empty-by-default per-deployment exemption used by tests for the fixture origin; private ranges (10/8, 169.254/16 metadata, etc.) are never exemptible through it.
- Automation-only-when-insufficient (guide/11) is enforced in the EXECUTOR (application layer) at the BrowserOpenAction dispatch: authorized workflow = `approved_tools` contains browser.open; static-fetch-insufficient = a prior failed web.fetch of the same URL (persisted summary match). Adapters stay policy-free.
- Download quarantine is capture-not-serve: the port (open/observe/interact) has no download surface; downloads land in the quarantine dir (size-capped post-completion — Playwright has no progress hook) and are reported for the normal ingestion controls, never auto-trusted.

## D17 (2026-09-22): Playwright browser E2E is a mandatory verification gate after every plan item
User order: "schedule regular playwright browser e2e testing after every plan item". From T30 onward, Atlas (or the task worker where delegated) must run a real-browser Playwright pass against the live stack (:5173) after each completed plan item, covering the task-touched user surface, saving evidence to scratch/t-qa/. Static gates alone are insufficient verification for UI-affecting work.
